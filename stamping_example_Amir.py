...
YP = 300.0#Mpa
velocity = 8000 #mm/s
#create a list of die diematers for parametric study
DDList = [50,52,54,56] #mm
ARList = []      #store list of spring back angle
for i in range(0,len(DDList)):
    #in each loop, using different die diameter:
    DbtwDies = DDList[i]#mm
...
#==== create x-y data ##Die Diameter - spring back angle for all loops====
MkList = [HOLLOW_CIRCLE, HOLLOW_SQUARE, HOLLOW_DIAMOND, HOLLOW_TRI,
    CROSS, XMARKER, POINT]
dataResult= session.XYData(data=zip(DDList, ARList), name='Springback',
    xValuesLabel='Die Diameter(mm/s)', yValuesLabel='Spring back angle')
...
#========================generate Die diameter - angle plot====================
...
session.printOptions.setValues(rendition=GREYSCALE, vpDecorations=OFF, 
    reduceColors=False)
session.printToFile(fileName='SpringbackCompareDiameter', format=PNG, 
    canvasObjects=( vp, ))